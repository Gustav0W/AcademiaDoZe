...
public class MatriculaService : IMatriculaService
{
    ...
    {
        var matriculaExistente = await _repoFactory().ObterPorId(matriculaDto.Id) ?? throw new KeyNotFoundException($"Matrícula ID {matriculaDto.Id} não encontrada");
    ...
    public async Task<IEnumerable<MatriculaDTO>> ObterPorAlunoIdAsync(int alunoId)
    {
        ...
        var matriculasDto = matriculas.Select(m => m.ToDto());
    ...
    {
        var matricula = await _repoFactory().ObterTodos();
        return [.. matricula.Select(m => m.ToDto())];
    }
